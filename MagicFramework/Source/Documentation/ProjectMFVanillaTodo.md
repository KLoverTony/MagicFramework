...
- Late research nodes can remain as intentional discipline/future-content scaffolding, but their release text should make the current payoff honest: `MFV_GrandSorcery` primarily unlocks the broad Archmage discipline for now, `MFV_Chronomancy` now has a verified first pass through `Temporal Resurrection` and `Borrowed Season`, `MFV_Illusion` now has a verified first pass through `Phantom Reinforcements`, `MFV_Fleshcraft` should build from the undead/construct lifecycle support toward golems, and `MFV_InfernalPact` still needs a core concept.
...
- concrete follow-up tasks for underused research nodes, especially Fleshcraft, Infernal Pact, and Grand Sorcery; Chronomancy and Illusion should stay on balance/icon/text/watch after their first content passes
