...
- Aquamancy is the most obvious school-content gap now that Forbidden Lore, Geomancy, Vitalism, planar magic, and enchanted weapons all have validated first-pass content. The next bounded content slice should be `Desiccate`/`Dessicate`: an offensive water spell that weaponizes dehydration rather than repeating frost or lightning.
...
- Spell balance: Aquamancy should be the next school-content pass now that Dominate Will/Forbidden Plague give Forbidden Lore its first identity pass, Dig/Earth Wall start Geomancy follow-up, and Cure Disease adds a Vitalism support beat. Start with Desiccate as a hostile attack spell.
...
| MF-046B | P1 | M | MFVanilla/Spells | Next content | Implement Desiccate/Dessicate as the next Aquamancy attack spell now that Dig and Earth Wall passed smoke testing. See [ProjectMFVanillaTodo.md](ProjectMFVanillaTodo.md#mf-046b-geomancy-and-aquamancy-follow-up). |
...
1. Implement Desiccate/Dessicate as a bounded Aquamancy attack spell and decide the final spelling before authoring defs.
