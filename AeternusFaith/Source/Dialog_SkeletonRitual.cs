...
{
    public class Dialog_SkeletonRitual : Window
    {
        private readonly Thing lectern;
        private readonly Thing circle;
        private readonly Action<Pawn, List<Pawn>, Corpse, Thing> startRitual;
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            Widgets.Label(new Rect(inRect.x, inRect.y, inRect.width, 34f), "Skeleton rite");
            ...
            bool canStart = corpse != null && conductor != null && conductorValidator(conductor, corpse, circle);
            if (!canStart)
                GUI.color = Color.gray;

            if (Widgets.ButtonText(buttonRect, "Begin rite") && canStart)
            {
                startRitual(conductor, audience.ToList(), corpse, circle);
                Close();
            }
        ...
        private void DrawConductorSelector(Rect viewRect, ref float y)
        {
            ...
            foreach (Pawn candidate in EligiblePawns)
            {
                ...
                bool valid = corpse != null && conductorValidator(candidate, corpse, circle);
